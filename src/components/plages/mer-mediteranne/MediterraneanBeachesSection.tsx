...
const MediterraneanBeachesSection: React.FC = () => {
  ...
  // Get unique cities
  const cities = ['ALL', ...new Set(mediterraneanBeaches.map((beach: MediterraneanBeach) => beach.location.city))];

  const filteredBeaches = mediterraneanBeaches.filter((beach: MediterraneanBeach) => {
    // Search filter
